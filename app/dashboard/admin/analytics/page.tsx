          ...
          {statsCards.map((stat, index) => {
            ...
            return (
              <motion.div
                ...
                className={`glass-card p-6 relative overflow-hidden group hover:shadow-lg ${colors.glow}`}
              >
                <div className="flex items-start justify-between mb-4">
                  <div className={`w-12 h-12 rounded-xl ${colors.bg} flex items-center justify-center`}>
                    <stat.icon className={`w-6 h-6 ${colors.text}`} />
                  </div>
                  <div className={`flex items-center gap-1 text-sm font-medium ${stat.isPositive ? 'text-emerald-400' : 'text-rose-400'}`}>
